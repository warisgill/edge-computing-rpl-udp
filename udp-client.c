...
PROCESS_THREAD(udp_client_process, ev, data){
    ...
    /*
        setting destination IP address to the python gateway
    */
    ...
    while(1) {
        ...
        if(NETSTACK_ROUTING.node_is_reachable()) {    
            /* Send to DAG root */
            count = count; //SEND_INTERVAL- CLOCK_SECOND + (random_rand() % (2 * CLOCK_SECOND));
            LOG_INFO("Sending request %u to ", count);
            LOG_INFO_6ADDR(&dest_ipaddr);
            LOG_INFO_("\n");
            snprintf(buffer, sizeof(buffer), "hello %d", count);
